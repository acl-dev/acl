...
static int rfc1035HeaderPack(char *buf, size_t sz, rfc1035_message * hdr)
{
	...
	if (sz < 12)
		msg_fatal("%s: sz(%d) < 12", __FUNCTION__, (int) sz);
	...
	off += sizeof(s);
	if (off != 12)
		msg_fatal("%s: off(%d) != 12", __FUNCTION__, off);
	return off;
}
...
static int rfc1035LabelPack(char *buf, size_t sz, const char *label)
{
	...
	if (sz < len + 1)
		msg_fatal("%s: sz(%d) < len(%d) + 1",
			__FUNCTION__, (int) sz, (int) len);
	*(buf + off) = (char) len;
	off++;
...
static int rfc1035NamePack(char *buf, size_t sz, const char *name)
{
	...
	*/
	...
	if (off > (int) sz)
		msg_fatal("%s: off(%d) > sz(%d)", __FUNCTION__, off, (int) sz);
	return off;
}
	...
	unsigned short type, unsigned short tclass)
{
	...
	off += sizeof(s);
	if (off > (int) sz)
		msg_fatal("%s: off(%d) > sz(%d)", name, off, (int) sz);
...
static int rfc1035HeaderUnpack(const char *buf, size_t sz, int *off,
	rfc1035_message * h)
{
	const char *name = "rfc1035HeaderUnpack";
	unsigned short s;
	unsigned short t;

	if (*off != 0)
		msg_fatal("%s: *off(%d) != 0", name, *off);
	...
	unsigned short *rdlength, char *name, size_t ns, int rdepth)
{
	...
	if (ns <= 0)
		msg_fatal("%s: ns(%d) <= 0", __FUNCTION__, (int) ns);
	do {
		if (*off >= (int) sz)
			msg_fatal("%s: *off(%d) >= sz(%d)",
				__FUNCTION__, *off, (int) sz);
		c = *(buf + (*off));
		if (c > 191) {
			/* blasted compression */
			unsigned short s;
	...
	if (no > (int) ns)
		msg_fatal("%s: no(%d) > ns(%d)", __FUNCTION__, no, (int) ns);
	return 0;
}
...
static void rfc1035RRDestroy(rfc1035_rr * rr, int n)
{
	if (rr == NULL)
		return;
	if (n <= 0)
		msg_fatal("%s: n(%d) <= 0", __FUNCTION__, n);
	while (n--) {
		if (rr[n].rdata)
			free(rr[n].rdata);
	}
...
int rfc1035MessageUnpack(const char *buf, size_t sz, rfc1035_message **answer)
{
	...
	*answer = NULL;
	...
	querys = msg->query = (rfc1035_query*) calloc((int) msg->qdcount, sizeof(*querys));
	...
	*answer = msg;
	recs = msg->answer = (rfc1035_rr*) calloc((int) msg->ancount, sizeof(*recs));
...
ssize_t rfc1035BuildAReply(const char *hostname, const ARGV *ip_argv,
	...
	unsigned short qid, char *buf, size_t sz)
{
	...
	if (h.nscount) {
		memset(&rr, 0, sizeof(rr));
		snprintf(rr.name, sizeof(rr.name), "%s", domain_root);
		rr.type = RFC1035_TYPE_NS;
		rr.tclass = RFC1035_CLASS_IN;
		rr.ttl = 600;
		rr.rdlength = (unsigned short) strlen(dnsname);
		rr.rdata = strdup(dnsname);
		offset += rfc1035RRPack(&rr, buf + offset, sz - offset);
		free(rr.rdata);
	}
	...
	unsigned short qid, rfc1035_query * query)
{
	...
	if (offset > sz)
		msg_fatal("%s: offset(%d) > sz(%d)",
			__FUNCTION__, (int) offset, (int) sz);
