...
{
public:
	pgsql_oper(acl::db_pool& dbp, unsigned long id)
		: dbp_(dbp), id_(id)
	{
	}
...
class pgsql_thread : public acl::thread
{
public:
	pgsql_thread(int id, acl::db_pool& dbp, const char* oper, int count)
...
protected:
	void* run(void)
	{
		pgsql_oper dboper(dbp_, id_);

		if (oper_.equal("add", false))
			dboper.pgsql_add(count_);
		else if (oper_.equal("get", false))
			dboper.pgsql_get(count_);
		else
...
class pgsql_fiber : public acl::fiber
{
public:
	pgsql_fiber(int id, acl::db_pool& dbp, const char* oper, int count)
		: id_(id), dbp_(dbp), oper_(oper), count_(count) {}
	~pgsql_fiber(void) {}
...
int main(int argc, char *argv[])
{
	...
	acl::string pgsql_path("../../lib/libpg.so");
	acl::string dbaddr("127.0.0.1:5432"), dbname("acl_db");
	...
	// setup libpgsqlclient_r.so path
	acl::db_handle::set_loadpath(pgsql_path);

	// init pgsql connection configure
	...
	// init pgsql connections pool
	acl::pgsql_pool dbpool(dbconf);
	...
	if (use_threads)
	{
		...
		for (int i = 0; i < cocurrent; i++)
		{
			acl::thread* thread = new
				pgsql_thread(i, dbpool, oper, count);
	...
	else
	{
		...
		{
			acl::fiber* f = new pgsql_fiber(i, dbpool, oper, count);
			f->start();
		}
