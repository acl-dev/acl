...
/**
 * When checking for memory leaks in memory allocation in acl library, by overloading new/delete
 * to record the creation and destruction process of objects. If an object is created but not released, it will be internally marked as
 * a memory leak. Users can periodically output these address information to see which objects created in which files were not
 * released. To use this feature, need to enable the macro "//#define ACL_CPP_DEBUG_MEM" in lib_acl_cpp/include/acl_cpp/acl_cpp_define.hpp,
...
/**
 * Display current memory status to screen. Users can add timers to periodically call this function
 */
 ...
 * @return {void*} Allocated memory address. If memory allocation fails, directly abort
 */
