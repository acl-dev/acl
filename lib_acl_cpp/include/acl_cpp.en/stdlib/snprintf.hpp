...
/**
 * Standard C snprintf API wrapper, ensures the last byte in the result buffer is '\0'
 ...
 * @return {int} When buffer length is sufficient, returns actual copied data length, otherwise:
 *  1) On UNIX/LINUX platform, returns the actual required buffer length, i.e., when buffer length is insufficient, return value
 *     >= size; note that the meaning of this return value differs from that on _WIN32
