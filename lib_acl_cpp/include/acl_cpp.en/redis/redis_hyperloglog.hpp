...
public:
	 ...
	 *  the key
	 * @param first_element {const char*} First element value in the element set,
	 * non-empty string
	 *  the first element of the elements list, and the last must be NULL
	 *  in the elements list
	 * @return {int} Whether the operation was successful, and indicates whether
	 * changes occurred. Return value meanings:
	 *  return the follow values:
	 * 1: Operation successful, and data was changed (new data added or old data
	 * changed)
	...
	/**
	 * Get the approximate number of deduplicated elements in the given key list's
	 * HyperLogLog
	 ...
	 * @return {int} Approximate number of deduplicated elements in the key list
	 * set
	 */
	...
	/**
	 * Merge multiple HyperLogLogs into one HyperLogLog. The cardinality of the
	 * merged
	 * HyperLogLog is approximately the union of the visible sets of all input
	 * HyperLogLogs
	 ...
	 * @param first_src {const char*} Key of the first source HyperLogLog object in
	 * the source object set
	 *  the first source key which must not be NULL in the sources list,
	 *  and the last one must be NULL showing the end of the list
	 * @return {bool} Whether the operation was successful. Returns false if error
	 * or destination/source objects are not
	 *  HyperLogLog objects
