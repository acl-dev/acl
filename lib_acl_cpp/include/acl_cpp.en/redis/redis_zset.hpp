...
public:
	 ...
	 *  0: Indicates nothing was added, possibly because member already exists and score was updated.
	 ...
	 *  -1: Error occurred or data object corresponding to key is not a valid sorted set object.
	 ...
	 *  0: No members in specified score range corresponding to this key sorted set, or KEY sorted set's corresponding score range members are empty.
	...
	/**
	 * Get member list at specified position range in key sorted set. Members are sorted by score value in ascending order.
	 ...
	 * @param result {std::vector<string>*} When not empty, stores result. Internally first calls
	 ...
	 *  Note: About subscript position: 0 indicates first member, 1 indicates second member, -1 indicates last member.
	...
	/**
	 * Get member-score list at specified position range in key sorted set. Members are sorted by score value in ascending order.
	 * @param key {const char*} Sorted set key value.
	 * @param start {int} Start subscript position.
	 * @param stop {int} End subscript position (including this position).
	 * @param out Stores "member-score" pair collection. Internally first calls out.clear().
	 * @return {int} Number of members in result.
	 *  0: Indicates result is empty or key does not exist.
	 * -1: Indicates error occurred or key is not a sorted set object.
	 * >0: Number of result members.
	...
	/**
	 * In sorted set key, get members with score values between min and max (including min and max).
	 * Sorted set members are sorted by score value in ascending (small to large) order.
	 * @param key {const char*} Sorted set key value.
	 * @param min {double} Minimum score.
	 * @param max {double} Maximum score.
	 * @param out {std::vector<string>*} When not empty, stores member result collection.
	 * @param offset {const int*} When not empty, indicates start subscript of result collection.
	 * @param count {const int*} When not empty, indicates number of result members to get.
	 * @return {int} Number of members in result.
	 *  0: Indicates result is empty or key does not exist.
	 * -1: Indicates error occurred or key is not a sorted set object.
	 * >0: Number of result members.
	 ...
	 * 3. By default, range value uses closed interval (less than or equal to or greater than or equal to). You can also add
	 *   opening parenthesis ( before value to use optional open interval (less than or greater than). For example:
	...
	/**
	 * In sorted set key, get member-score pairs with score values between min and max (including min and max).
	 * Sorted set members are sorted by score value in ascending (small to large) order. Values (min/max) use
	...
	/**
	 * Get rank (subscript starts from 0) of member member in sorted set key. Sorted set members are sorted by score
	 * value in ascending (small to large) order.
	 * @param key {const char*} Sorted set key value.
	 * @param member {const char*} Member name.
	 * @param len {size_t} Length of member.
	 * @return {int} Subscript position value. -1 -- Error occurred, key is not a sorted set object, or member does not exist.
	 */
	 ...
	 * @param key {const char*} Sorted set key value.
	 * @param first_member {const char*} First member in list of members to be removed.
	 * @return {int} Number of successfully removed members. -1 indicates error occurred or key is not a sorted set object.
	 *  0 indicates sorted set does not exist or member does not exist. > 0 indicates number of successfully removed members.
	 */
	 ...
	 * You can directly use subscripts start and stop to specify range. start and stop are included,
	 * subscript range starts from 0, meaning 0 indicates first member in sorted set,
	 * 1 indicates second member in sorted set, and so on.
	 * You can also use negative subscripts, where -1 indicates last member, -2 indicates second-to-last member, and so on.
	 * @param key {const char*} Sorted set key value.
	 ...
	 *  0: Indicates key does not exist or no members were removed (range does not exist).
	 * -1: Indicates error occurred or key is not a sorted set object.
	 */
	int zremrangebyrank(const char* key, int start, int stop);

	/**
	 * Remove members with score values between min and max (including min and max) in sorted set key. Starting from version 2.1.6, members with score values equal to min and max can also be excluded. For details, see ZRANGEBYSCORE command.
	 * @param key {const char*} Sorted set key value.
	 * @param min {double} Minimum score.
	 * @param max {double} Maximum score.
	 * @return {int} Number of successfully removed members. -1 indicates error occurred or key is not a sorted set object.
	 ...
	 * Remove members with score values between min and max (including min and max) in sorted set key. Starting from version 2.1.6, members with score values equal to min and max can also be excluded. For details, see ZRANGEBYSCORE command.
	 * @param key {const char*} Sorted set key value.
	 * @param min {const char*} String format minimum score. For details, see zrangebyscore notes.
	...
	/**
	 * Get member list at specified position range in key sorted set. Members are sorted by score value in descending order.
	 * @param key {const char*} Sorted set key value.
	 * @param start {int} Start subscript position.
	 * @param stop {int} End subscript position (including this position).
	...
	/**
	 * Get member-score list at specified position range in key sorted set. Members are sorted by score value in descending order.
	 * @param key {const char*} Sorted set key value.
	 * @param start {int} Start subscript position.
	 * @param stop {int} End subscript position (including this position).
	 * @param out Stores "member-score" pair collection. Internally first calls out.clear().
	...
	/**
	 * In sorted set key, get members with score values between min and max (including min and max).
	 * Sorted set members are sorted by score value in descending (large to small) order.
	 * @param key {const char*} Sorted set key value.
	 * @param min {const char*} String representation of minimum score.
	 * @param max {const char*} String representation of maximum score.
	 * @param out {std::vector<string>*} When not empty, stores member result collection.
	...
	/**
	 * In sorted set key, get member-score pairs with score values between min and max (including min and max).
	 * Sorted set members are sorted by score value in descending (large to small) order. Values (min/max) use
	 * numeric representation.
	 * @param out Stores score-member pair result. Internally first calls out.clear().
	 * @param count {const int*} When not empty, indicates number of result members to get.
	 */
	 ...
	 * @return {bool} Returns false when member does not exist or error occurred. Otherwise returns true.
	 */
	...
	/**
	 * Compute union of one or more sorted sets. For given key sorted sets, compute union of numkeys specified
	 * sorted sets and store result (aggregation) in destination sorted set. By default, when a member's score
	 * value exists in multiple sorted sets, sum of scores of this member in all sorted sets is used as
	 * this member's score value in result.
	 * @param dst {const char*} Destination sorted set key value.
	 * @param keys Source sorted set key value-weight collection. When using weight option, you need to
	 *  specify a multiplication factor for each sorted set respectively. Each sorted set's all members' score
	 *  values need to be multiplied by this sorted set's factor before being passed to aggregation function.
	 *  If WEIGHTS option is not specified, multiplication factor defaults to 1.
	 * @param aggregate {const char*} Aggregation method. Default is SUM aggregation method. Aggregation methods are as follows:
	 *  SUM: Sum of score values of a certain member in all sorted sets is used as this member's score value in result sorted set.
	 *  MIN: Minimum score value of a certain member in all sorted sets is used as this member's score value in result sorted set.
	 *  MAX: Maximum score value of a certain member in all sorted sets is used as this member's score value in result sorted set.
	 * @return {int} Number of elements (members) stored in destination sorted set result. When source
	 *  sorted sets have duplicate members, only one member is stored. Returns -1 to indicate error.
	 */
	...
	/**
	 * Compute intersection of one or more sorted sets. For given key sorted sets, compute intersection of numkeys specified
	 * sorted sets and store result (aggregation) in destination sorted set. By default, when a member's score
	 * value exists in multiple sorted sets, sum of scores of this member in all sorted sets is used as
	 * this member's score value in result.
	 * @return {int} Number of elements (members) stored in destination sorted set result.
	 */
	...
	/**
	 * Incrementally iterate elements in sorted set, including element members and element scores.
	 * @param cursor {int} Cursor value. Write 0 when starting iteration.
	 * @param out Stores result. Internally uses append method. Cursor iterates and appends results to collection.
	 *  To prevent results from growing too large, users should clear this parameter before each call.
	 * @param pattern {const char*} Matching pattern with glob style, effective when not empty.
	 * @param count {const size_t*} Limit maximum number of results. Effective only when pointer is not empty.
	...
	/**
	 * When all members in sorted set have the same score, sorted set elements will be sorted by member name value
	 * lexicographical ordering (dictionary order). This function queries members in sorted set key
	 ...
	 * @param offset {const int*} When not empty, effective. Start subscript value for selecting from result.
	 * @param count {const int*} When not empty, effective. Number of elements to select from result at specified subscript position.
	 * @return {int} Number of members in result.
	 *  0: Indicates result is empty or key does not exist.
	 * -1: Indicates error occurred or key is not a sorted set object.
	 * >0: Number of result members.
	 * Note: About range selection:
	 * 1. Legal min and max strings include opening parenthesis ( and closing bracket [. Opening parenthesis ( indicates open interval (specified value
	 *   will not be included in range), and [ indicates closed interval (specified value will be included in range).
	 * 2. Special values + and - can be used in min string and max string. + means positive infinity, and - means negative infinity. Therefore, when all members in a sorted set have the same score, sorted set
	...
	/**
	 * When all members in a sorted set have the same score, for sorted set key, this function returns number of
	...
	/**
	 * When all members in a sorted set have the same score, for sorted set key, this function removes all
	 * elements (members) with values between min and max in this set.
