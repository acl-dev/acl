...
public:
	...
	/**
	 * Watch one (or more) key(s). If this (or these) key(s) are modified by other commands before transaction execution,
	 ...
	 * @return {bool} Whether operation was successful. Even if some keys in key collection do not exist, will still return success
	...
	/**
	 * Mark the start of a transaction block. Multiple commands within the transaction block will be placed in a queue in order,
	...
	/**
	 * Execute all commands within the transaction block. If some key(s) are being monitored by WATCH command,
	 * and there are commands related to this (or these) key(s) in the transaction block, then EXEC command will only execute and take effect
	 * when this (or these) key(s) have not been modified by other commands, otherwise the transaction is aborted;
	 * After successfully executing this command, can call get_size()/get_child() below to get operation result of each command
	...
	/**
	 * Cancel transaction, abandon execution of all commands within the transaction block. If currently using WATCH command to monitor some key(s),
	...
	/**
	 * Can call this function multiple times between multi and exec to execute multiple redis client commands
	...
	/**
	 * Call this function multiple times between multi and exec to execute multiple redis client commands
	...
	/**
	 * After successfully calling exec, call this function to get the length of operation result array
	...
	/**
	 * Get execution result object of the command corresponding to the specified index
	 * get the result of the given subscript
	 * @param i {size_t} Index of command execution result in result array
	 *  the given subscript
	 * @param cmd {string*} When this parameter is not NULL, stores the corresponding redis command
	 *  if not NULL, it will store the command of the given subscript
	 * @return {const redis_result*} Result of executing a command. Returns NULL when i is out of bounds
