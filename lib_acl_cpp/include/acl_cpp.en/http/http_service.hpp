...
public:
	/**
	 * Constructor
	 * @param domain {const char*} Domain name of HTTP server (can also be IP), non-empty.
	...
	/**
	 * Get HTTP request body data. This function will be called in a loop during request process until returned data
	 * object's data is empty
	 * @return {const string*} Request body result data. If returns NULL pointer or returned buffer
	 *  object's data is empty (i.e., string->empty()), it indicates HTTP request body data ends
	 * Note: Different from other functions, this virtual interface is called in another child thread, so if subclass
	 * implements this interface and needs to call resources that compete with original thread, should pay attention to lock protection
	 */
	 ...
	 * @param hdr {const HTTP_HDR_RES*} HTTP response header. Structure definition see:
	 *  acl_project/lib_protocol/include/http/lib_http_struct.h
	 */
	virtual void on_hdr(const char* addr, const HTTP_HDR_RES* hdr) = 0;

	/**
	 * Callback interface when HTTP response body from HTTP server is obtained. When HTTP response body data
	 * is relatively large, this callback will be called multiple times until error (will call on_error) or data is read completely.
	 * When both parameters of this callback are 0, when data and dlen are both 0, it indicates reading HTTP
	 * response body ends
	 ...
	 * Note: If HTTP response only has header data and no body data, this function will also be called to notify user
	 *     that HTTP session ends
	 */
	virtual void on_body(const char* data, size_t dlen) = 0;

	/**
	 * If error occurs during HTTP request or response process, this interface will be called to notify subclass of error.
...
public:
	 ...
	 *  blocking main thread's _WIN32 message loop due to task thread sending data messages too fast,
	 *  automatically sleeps for milliseconds when task thread sends data messages. For other asynchronous engines,
	...
	/**
	 * Application calls this function to start HTTP session process. http_service class object is responsible
	 * for asynchronously sending HTTP request to server, and asynchronously reading response from HTTP server
...
protected:
	...
	/**
	 * Base class virtual function. Callback function after listening stream is successfully opened
