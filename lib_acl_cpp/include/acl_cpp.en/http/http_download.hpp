...
public:
	...
	/**
	 * Before calling run, can get request header object through this function,
	 * convenient for users to set
	 * their own request header fields (but set_method/set_range are automatically
	 * set internally)
	...
	/**
	 * Calling this function can get http_request object, convenient for setting or
	 * querying request header
	...
	/**
	 * Download file. When range_from >= 0 and range_to >= range_from,
	 * automatically
	 * uses segmented download mode, otherwise uses full download mode
	 * @param range_from {acl_int64} Download start offset position, index starts
	 * from 0.
	 * When this value >= 0 and range_to >= this value, segmented download mode is
	 * used
	 ...
	 * @return {bool} Whether download was successful. If returns true, it
	 * indicates download was successful. Otherwise,
	 * may be input parameters are illegal, or URL does not exist, or server does
	 * not support resumable transfer, or
	 ...
	 * @param url {const char*} When not empty, uses this URL to replace URL input
	 * in constructor,
	...
	/**
	 * Get server address obtained from url input by constructor or reset function,
	 * format:
	 * ip[|domain]:port
	 * @return {const char*} Returns NULL indicates input url is illegal
	 */
...
protected:
	/**
	 * Callback function after sending HTTP request data and reading HTTP server
	 * response header
	...
	/**
	 * During download process, notify subclass of downloaded data and data length
	 * while downloading
