...
/**
 * HTTP client request class, supports connection reuse, and automatically reconnects when connection times out.
 */
...
public:
	 ...
	 *  automatically opens. At this time, this value indicates connection server timeout time (seconds).
	 *  Network IO read/write timeout time is inherited from base class.
	 * @param unzip {bool} Whether to automatically decompress server response body.
	...
	/**
	 * Constructor. This constructor internally creates socket_stream object and closes it.
	...
	/**
	 * Set client SSL communication mode. Internal default is non-SSL communication mode.
	...
	/**
	 * Set SNI field when SSL handshake. After calling this API to set SSL SNI field, when calling this object's
	 * reset() API, it will automatically restore this API's set value. If you need to modify it, you need to call
	 * this function again. When this parameter is empty, corresponding field will be automatically set.
	...
	/**
	 * Set SNI field prefix when SSL handshake. After calling this API to set SSL SNI field prefix,
	 * when calling this object's reset() API, it will automatically restore this API's set value. If you need to modify it,
	 * you need to call this function again. When this parameter is empty, corresponding field will be automatically set.
	...
	/**
	 * Set SNI field suffix when SSL handshake. After calling this API to set SSL SNI field prefix,
	 * when calling this object's reset() API, it will automatically restore this API's set value. If you need to modify it,
	 * you need to call this function again. When this parameter is empty, corresponding field will be automatically set.
	...
	/**
	 * Get HTTP request header, and then you can add fields to returned HTTP request header object
	...
	/**
	 * Set local character set. When local character set is not empty, boundary data boundary string conversion.
	...
	/**
	 * Send HTTP request including HTTP request header and HTTP request body, and simultaneously
	 * read HTTP response header from HTTP server. For long connections, when timeout occurs
	 * you can call this function again. Before calling get_body function, you must
	 * call this function (or write_head/write_body).
	 * Generally, after this function sends request data and reads HTTP response header,
	 ...
	 * @param data {const void*} Data to be sent. When not empty, automatically determines
	 *  POST request type, otherwise GET request type.
	 * @param len {size_} When data is not empty, indicates data length.
	 * @return {bool} Whether sending request data and reading HTTP response header was successful.
	 */
	...
	/**
	 * When using streaming write mode, you need to call this function first to send HTTP request header.
	 * @return {bool} Whether successful. Only after success can you continue calling write_body.
	 */
	bool write_head();

	/**
	 * When using streaming write mode, after calling write_head, you can call this function in a loop
	 * to send HTTP request body data. When data parameter is empty pointer, it indicates data sending is complete;
	 * After sending request data, this function internally automatically reads HTTP response header data. Users can
	 * then call get_body/read_body to read HTTP response body.
	 * @param data {const void*} Data address pointer. When parameter value is empty pointer, it indicates
	 ...
	 *  Note: After application sends data, you must call this function once more, and data parameter must be empty.
	 */
	...
	/**
	 * After sending request data, internally automatically calls HTTP response header reading process. You can get server response HTTP status code (2xx, 3xx, 4xx, 5xx) through this function.
	 * Actually, this function internally only calls http_client::response_status function.
	 * @return {int}
	 */
	...
	/**
	 * After request succeeds, call this function to get complete server response body
	 ...
	 * Note: When response body is very large, you should use this function to avoid memory overflow.
	 */
	 ...
	 * and store it in user buffer. Call this function in a loop until data is finished.
	 ...
	 *  or connection closed, > 0 indicates number of bytes read. Users should continue calling until
	 *  return value <= 0.
	 *  Note: This function returns original HTTP response body data, without decompression or character
	 *  encoding conversion. Users need to handle it themselves.
	 */
	int read_body(char* buf, size_t size);

	/**
	 * After request succeeds, call this function to read HTTP response body. Call in a loop
	 * to read data. Internally automatically decompresses compressed data. If set_charset function was called before calling this function
	 * to set local character set, data will be converted to local character set.
	 * @param out {string&} Store result.
	 * @param clean {bool} Whether to internally automatically clear out buffer each time this function is called.
	 *  buffer.
	 * @param real_size {int*} When pointer is not empty, stores uncompressed response body
	 *  length. When constructor specified automatic decompression mode and response body length > 0, this
	 *  value stores length value should be same as this function's return value. When no data was read, this
	 *  value stores length value is 0.
	 * @return {int} == 0 indicates finished, but connection not closed. >0 indicates this time read
	 *  data length (when decompression is enabled, it indicates decompressed data length,
	 *  which may differ from actual response data. Actual response data length should be obtained through real_size
	 *  parameter); < 0 indicates error or connection closed. At this time, if real_size is not empty, real_size
	 *  stored value should be 0.
	 *  When return value is 0, you can call body_finish function to determine whether response body is finished.
	 */
	int read_body(string& out, bool clean = false, int* real_size = NULL);

	/**
	 * After request succeeds, call this function to read one line of data from HTTP server. Call in a loop
	 * to read data until it returns false or body_finish() returns true.
	 * Internally automatically decompresses compressed data. If set_charset function was called before calling this function
	 * to set local character set, data will be converted to local character set.
	 * @param out {string&} Store result.
	 ...
	 * @return {bool} Whether one line was read. When returns true, it means one line was read.
	 *  You can determine whether response body is completely read by whether body_finish() is true.
	 ...
	 * after reading response header, this function can get start offset position of range support function.
	 * @return {acl_int64} When server does not support range format, returns value < 0.
	 */
	__int64 get_range_from(void) const;

	/**
	 * When http_request::request_header().set_range() was called to read data,
	 * after reading response header, this function can get end offset position of range support function.
	 * @return {acl_int64} When server does not support range format, returns value < 0.
	 */
	...
	/**
	 * When http_request::request_header().set_range() was called to read data,
	 * after reading response header, this function can get total resource size of range support function (this value
	...
	/**
	 * Get http_client HTTP connection object used internally. You can use this object to read
	...
	/**
	 * Reset object state. When reusing the same connection object, call this function.
	 */
	void reset();

protected:
	/**
	 * Override connect_client virtual function. Explicitly call this function to open connection to server.
