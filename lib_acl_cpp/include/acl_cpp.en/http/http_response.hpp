...
public:
	/**
	 * Constructor: socket_stream stream object passed through this constructor needs to be deleted by user itself.
	 * Internally will not delete it during destruction.
	 * @param client {socket_stream*} Data connection stream, non-empty
	 * Note: This class instance can be used multiple times in long connection, but must pay attention to
	...
	/**
	 * Read HTTP request header from HTTP request client. After calling this method, can call
	...
	/**
	 * Read HTTP request body data in xml format. After calling this function, should call
	 * response to return response data to client. When this function is called each time, internal
	 * object will be initialized, so this function can be called multiple times in multiple sessions
	 * @param out {xml&} Store HTTP request body data in this xml object
	 * @param to_charset {const char*} When this item is not empty, internally automatically
	 ...
	 *      When request body data is particularly large, should not use this function to avoid running out of memory
	 */
	bool get_body(xml& out, const char* to_charset = NULL);

	/**
	 * Read HTTP request body data in json format. After calling this function, should call
	 * response to return response data to client. When this function is called each time, internal
	 * object will be initialized, so this function can be called multiple times in multiple sessions
	 ...
	 * Note: When request body data is particularly large, should not use this function to avoid running out of memory
	 ...
	 * @return {int} Return value == 0 indicates normally read complete, < 0 indicates client
	 *  closed connection, > 0 indicates data already read. User should keep reading data until
	 *  return value <= 0
	 *  Note: This function reads raw HTTP body data, does not decompress or character set
	 ...
	 * your own response header fields or http_header::reset() to reset response header state in returned HTTP response header object.
	 ...
	 * 1) Must first call read_header && get_body to get HTTP client's request data;
	 * 2) Must get http_header object through response_header, and set response header
	 ...
	 * 3) When using chunked mode to transfer data, should call this function once more at the end,
	 * with all parameters set to 0 to indicate data end
	 * @param data {const void*} Data address
	 * @param len {size_t} data data length
	 * @return {bool} Whether sending was successful. If returns false, it indicates connection interrupted
	 */
	 ...
	 * partial data of client request header through returned object. Reference: http_client class
