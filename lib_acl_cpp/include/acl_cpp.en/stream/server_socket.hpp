...
/**
 * Server listening socket class. Receives client connections and creates client stream connection objects
 */
class ACL_CPP_API server_socket : public noncopyable {
public:
#if 0
	/**
	 * Constructor. After calling this constructor, need to call class method open to listen on specified service address
	...
	/**
	 * Constructor. After calling this constructor, calling open method again is prohibited
	 * @param sstream {ACL_VSTREAM*} Externally created listening stream object. This class only uses
	 *  but does not release it. Application should close this listening object itself
	 */
	server_socket(ACL_VSTREAM* sstream);

	/**
	 * Constructor. After calling this constructor, calling open method again is prohibited
	 * @param fd {ACL_SOCKET} Externally created listening handle. This class only uses but does not release it.
	 ...
	 *  ip:port. In unix environment, can also be domain socket, format: /path/xxx. On
	 *  Linux platform, if domain socket address is: @xxx format, i.e., first character is @, then
	 *  internally automatically enables Linux abstract domain socket mode (abstract unix socket)
	 ...
	 * @param timeout {int} When this value >= 0, uses timeout mode to accept client connection.
	 *  If client connection is not obtained within specified time, returns NULL
	 * @param etimed {bool*} When this pointer is not NULL, if this function returns
	 *  NULL due to timeout, this value is set to true
	 * @return {socket_stream*} Returns NULL indicates accept failed or timeout. Returned stream object needs
	 *  to be deleted by user after use.
	 */
	socket_stream* accept(int timeout = -1, bool* etimed = NULL);

#if __cplusplus >= 201103L
	// Use c++11 shared_ptr method to get client stream object, use stream object more safely
	...
	/**
	 * After normally listening on server address, calling this function can get listening socket
	 * @return {int}
	 */
#if defined(_WIN32) || defined(_WIN64)
	...
	/**
	 * Set listening socket's deferred accept function, i.e., only return connection to application when client connection has data.
	 * Currently this function only supports Linux
	 * @param timeout {int} If client connection does not send data within specified time,
