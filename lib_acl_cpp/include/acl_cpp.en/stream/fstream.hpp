...
public:
	...
	/**
	 * Create new file in read/write mode. File attributes are 0700. If file does
	...
	/**
	 * Delete file corresponding to this class object from disk. This function can
	 * only correctly delete file when internally knows file path,
	...
	/**
	 * Rename current file to specified filename. For WINDOWS platform, need to
	 * close current file
	 ...
	 * @param path {const char*} When not NULL, stored as file path of this file
	 * handle
	 ...
	 * @param whence {int} Move direction: SEEK_SET (move from file start
	 * position),
	 * SEEK_CUR (move from current file pointer position), SEEK_END (move from file
	 * end forward)
	 ...
	 * @return {const char*} Returns NULL indicates file has not been opened yet or
	 * error occurred
	 */
	 ...
	 * @param exclude {bool} Whether locking method is exclusive lock. Default is
	 * exclusive lock. If this value
	 *  is false, it is shared lock
	 * @return {bool} Returns false indicates locking failed. Can check error
	 * reason through acl::last_serror
	 */
	bool lock(bool exclude = true);

	/**
	 * After file is opened, this method is used to try to lock file. If locking
	 * succeeds, returns true. If
	...
	/**
	 * After successfully calling lock or try_lock, can call this method to unlock
	 * file
