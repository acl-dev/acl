...
public:
	/**
	 * Constructor
	 * @param addr_rw {const char*} Listening address of handlersocket plugin on MySQL,
	 * format: ip:port. Note: This address is the read-write address of handlersocket
	 * @param addr_rd {const char*} Listening address of handlersocket plugin on MySQL,
	 * format: ip:port. Note: This address is the read-only address of handlersocket
	 * @param cache_enable {bool} Whether to automatically enable row object caching mechanism internally
	 * @param retry_enable {bool} Whether to retry when error occurs due to network reasons
	 */
	 ...
	 *  field names separated by delimiters ",; \t", e.g.: user_id,user_name,user_mail
