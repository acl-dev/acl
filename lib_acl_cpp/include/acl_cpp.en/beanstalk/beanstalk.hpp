...
/**
 * Message ID starts from 1 and increments (see beanstalkd source code job.c for details:
 ...
 *         j->r.id = next_id++;
 *     }
 * Message priority pri value range is 0 ~ 4,294,968,295 (maximum unsigned value). Smaller value
 * means higher priority. Highest priority is 0.
 * Message default maximum length is 65,535 (maximum short value). This value can be modified by beanstalkd command
 * line parameters. For details, see <beanstalk protocol specification.pdf> in doc/ directory.
 * All operations internally automatically reconnect and retry. Before operations, you need to call watch or use
 * operations. Internally automatically manages these operations. Generally, you need to call open process explicitly, and users should
 * call close after use, which will disconnect from beanstalkd server connection, and internally automatically releases
...
public:
	 ...
	 * @param delay {unsigned} Indicates how many seconds job needs to wait before entering ready state.
	 * @param ttr {unsigned} Indicates how many seconds a worker has to execute this message.
	...
	/**
	 * Send message to selected tube (default tube if not selected) in formatted string format.
	 * @param pri {unsigned} Priority. Smaller value means higher priority.
	 * @param delay {unsigned} Indicates how many seconds job needs to wait before entering ready state.
	 * @param ttr {unsigned} Indicates how many seconds a worker has to execute this message.
	...
	/**
	 * Send message to selected tube (default tube if not selected) in formatted string format. All
	 ...
	 * @return {unsigned} Return value is number of watched message tubes. Return value > 0 means success.
	 */
	 ...
	 * @param tube {const char*} Message tube name.
	 * @return {unsigned} Return value is number of remaining watched message tubes. Return value > 0 means
	 *  success (if you need to watch a default message tube, after correctly calling this function, return value will be 1).
	 ...
	 * @param buf {string&} Buffer to store obtained message. Internally first clears this buffer.
	 * @param timeout {int} Timeout value for waiting for available messages. When -1,
	 *  it waits indefinitely. When > 0, if no message is available within this time, returns.
	 *  When == 0, immediately returns if no message is available.
	 * @return {unsigned long long} Returns obtained message number. Return value > 0
	 *  means correctly got a message. Otherwise means timeout or no message available. When
	 *  return value is 0, you can call get_error() to get error. When TIMED_OUT, it means
	 *  timeout. When DEADLINE_SOON, it means message has been reserved and within specified ttr
	...
	/**
	 * Put a message that has been reserved back to ready state (change job state to "ready").
	 * This message can be obtained by other workers.
	 * @param id {unsigned long long} Message number.
	 * @param pri {unsigned} Priority.
	 * @param delay {unsigned} How many seconds to wait before this message enters ready state.
	 * @return {bool} Whether successful.
	 */
	...
	/**
	 * Change a message state to "buried". Buried messages are stored in a FIFO queue,
	 * and these messages will not be processed by workers until client calls kick function.
	 * @param id {unsigned long long} Message number.
	...
	/**
	 * Extend time for a worker to reserve a message for execution. This is very useful for messages that need
	 * long time to complete. It can also be used to transfer a message from one worker that cannot complete it
	 * to another worker. A worker can extend time by calling this function multiple times
	 * during job execution (e.g., when receiving DEADLINE_SOON, you can call this function to extend time).
	 * @param id {unsigned long long} Message number.
	 * @return {bool} Whether successful.
	 */
	...
	/**
	 * Open beanstalkd server connection. Generally, you don't need to explicitly call this function to open connection, because
	 * it automatically connects. You need to call this function to enable automatic reconnection.
	 * @return {bool}  Whether successful.
	 */
	bool open();

	/**
	 * Explicitly close beanstalkd server connection. When object is destroyed, it will attempt to call this close process,
	 * and this function internally releases tube_used_ and tubes_watched_.
	 */
	void close();

	/**
	 * Notify beanstalkd server to exit (server will close connection after receiving this command).
	 */
	void quit();

	/**
	 * Get message body for specified message number.
	 * @param buf {string&} Message body buffer to store obtained message. Internally first clears this buffer.
	...
	/**
	 * Move messages in buried or delayed state to ready state, only for currently used tube.
	 * @param n {unsigned} Indicates maximum number of messages to kick each time,
	 *  server will kick at most n messages.
	 * @return {int} Indicates number of messages actually kicked. Returns -1 to indicate error.
	 */
	int  kick(unsigned n);

	/**
	 * Get currently used message tube for this client.
	 * @param buf {string&} Buffer to store currently used message tube. Internally first clears this buffer.
	 ...
	 * @param buf {string&} Buffer to store result. Internally first clears this buffer.
	 * @return {bool} Whether successfully got.
	 */
