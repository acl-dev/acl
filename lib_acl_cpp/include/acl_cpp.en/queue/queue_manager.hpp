...
public:
	 ...
	 * @param no_cache {bool} When true, requires that KEY corresponding to this file in cache
	 *  must not exist. If exists, returns NULL indicating this file is locked. When this parameter is
	 *  false, can directly use object in cache
	 * @return {queue_file*} Queue file object. Returns NULL if error or does not exist
	 */
	...
	/**
	 * Move queue file to target queue. After move succeeds, file object's internal content will change
	 * @param fp {queue_file*} Queue file object
	 * @param queueName {const char*} Target queue name
	 * @param extName {const char*} File extension name
	 * @return {bool} Whether moving queue file was successful. If move fails, caller should call
	 *  close_file to close this queue file object. This file will be moved by scheduled scanning task
	 */
	...
	/**
	 * Delete this queue file from disk. After deletion succeeds, this queue file handle has been deleted and cannot be used again.
	 * Even if file deletion fails, this queue file object is also released, only file deletion from disk failed.
	...
	* @param key {const char*} Partial filename of queue file (excluding path and extension)
	...
	* @param fp {queue_file*} Queue file object
	* @return {bool} Whether adding was successful. If fails, it indicates this object or its corresponding key value
	...
	* @return {bool} Whether deletion was successful. If fails, it indicates this queue file object does not exist
	*/
	...
	* @return {queue_file*} Scanned queue file object. Returns empty indicates scanning complete
	*  or error occurred. Non-empty object must be deleted after use to release internally allocated resources
	*/
	queue_file* scan_next(void);

	/**
	* Parse queue name, filename (excluding path and extension parts), file extension name from file path
	...
	* @param partName {acl::string*} Store filename part of file (excluding path and extension)
	...
	*/
	...
	/**
	* Parse filename (excluding path and extension), file extension name from file name (including extension but excluding path)
	*/
	...
	*/
	...
	/**
	* Calculate queue subdirectory path (represented as number) based on partial filename (excluding directory and extension)
