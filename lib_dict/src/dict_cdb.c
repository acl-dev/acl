...
static const char *dict_cdbq_lookup(DICT *dict, char *name, size_t name_len, char **value, size_t *size)
{
...
static DICT *dict_cdbq_open(const char *path, int dict_flags)
{
	...
	if ((fd = open(cdb_path, O_RDONLY)) < 0)
		acl_msg_fatal("%s(%d), %s: open database %s: %s",
			__FILE__, __LINE__, __FUNCTION__, cdb_path, acl_last_serror());
	...
	if (stat(path, &st) == 0
		...
		acl_msg_warn("database %s is older than source file %s", cdb_path, path);
...
static void dict_cdbm_update(DICT *dict, char *name, size_t name_len, char *value, size_t size)
{
	...
	if (strcasecmp(dict_cdbm->tmp_path, dict_cdbm->cdb_path) != 0) {
		if (rename(dict_cdbm->tmp_path, dict_cdbm->cdb_path) < 0)
			acl_msg_fatal("rename database from %s to %s: %s",
				dict_cdbm->tmp_path, dict_cdbm->cdb_path, acl_last_serror());
	}
...
static DICT *dict_cdbm_open(const char *path, int dict_flags)
{
	...
	for (;;) {
		if ((fd = open(tmp_path, O_RDWR | O_CREAT, 0644)) < 0
				|| fstat(fd, &st0) < 0)
			acl_msg_fatal("%s(%d), %s: open database %s: %s",
				__FILE__, __LINE__, __FUNCTION__, tmp_path, acl_last_serror());

		/*
		 * Get an exclusive lock - we're going to change the database so we
		 * can't have any spectators.
		 */
		if (acl_myflock(fd, ACL_INTERNAL_LOCK, ACL_MYFLOCK_OP_EXCLUSIVE) < 0)
	...
	if (cdb_make_start(&dict_cdbm->cdbm, fd) < 0)
		acl_msg_fatal("initialize database %s: %s", tmp_path, acl_last_serror());
