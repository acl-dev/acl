...
int acl_vstream_gets_peek(ACL_VSTREAM *fp, ACL_VSTRING *buf, int *ready)
{
	...
	*ready = 0;
	...
	/* XXX: ������ͨ����� *ready ֵ���ж��Ƿ��������, ϵͳIO����������
	 * ��ر�ʱ���ؽ������; ������� ACL_VSTRING_EOF �������Ӧ��ͨ��
	 * ��黺��������������δ�����������
	 */
...
static int loop_writen(ACL_VSTREAM *fp, const void *vptr, size_t size)
{
	...
	int   once_dlen = 64 * 1024 * 1024;  /* xxx: �� 64KB Ϊ��λд */
	...
	size_t buflen, int rw_timeout, int fdtype)
{
	...
	if (fdtype == ACL_VSTREAM_TYPE_FILE) {
		...
		/**
		 * bugfix: ��Ӧ�ڴ˴�ֱ����Ϊ��������ʽ�������Ӱ���ϲ������
		 * ����Ϊ��Ӧ����Ӧ���Լ���������¡�---zsx, 2016.11.18
		 */
		/* xxx: ���ڴ��ж�д��ʱ��������Ҫ�Ƚ� socket ��Ϊ������ģʽ��
		 * ������д�����ݰ�ʱ�������������ʱ����ʧЧ
		 */
		/**
...
acl_off_t acl_vstream_fseek(ACL_VSTREAM *fp, acl_off_t offset, int whence)
{
	...
	if (whence == SEEK_CUR) {
		...
		/* �Ƿ��ڶ����������� */
		if (fp->read_cnt >= offset) {
			/* ��ΪҪ�� fp->offset ƫ�� offset ���ֽں�, ��ָ��
			 * fp->read_ptr ��Ȼ�ڻ�������, ����ֻ��Ҫ�ƶ���ָ��
			 * �Ҽ��ٻ������ֽ��������� fp->offset ƫ��������.
			 */
			fp->read_cnt -= (int) offset;
			...
			/* ��ΪҪ����ӵ�ǰ��λ�� fp->offset ��ʼƫ�� offset
			 * ��λ��,�������л�����һ���Ļ�������(fp->read_cnt),
			 * ������Ҫ�ȴ� fp->offset ��ʼ�ƶ� fp->read_cnt
			 * ���ֽ�(�ƶ�����������),Ȼ�����ƶ�ʣ����ֽ�
			 * (�� offset - fp->read_cnt) ����; ��Ϊ�Ѿ��Ƴ�����
			 * ������������Ҫ�� fp->read_cnt �� 0.
			 */
			offset -= fp->read_cnt;
			fp->read_cnt = 0;
...
int acl_file_ftruncate(ACL_VSTREAM *fp, acl_off_t length)
{
	...
	/* �μ���C:\Program Files\Microsoft Visual Studio .NET 2003\Vc7\crt\src
	 * osfinfo.c
	 * _open_osfhandle: ��ACL_WINDOWS API���ļ����ת��Ϊ��׼C���ļ����
	 * _get_osfhandle: ���ݱ�׼C�ļ������ѯACL_WINDOWS API�ļ����
	 * _free_osfhnd: �ͷ��� _open_osfhandle �򿪵ı�׼C�ļ��������Դ��
	 *               ������ʵ�ʹرո�ACL_WINDOWS API��������Ի���Ҫ������ʵ
	 *               ACL_WINDOWS API�ļ�������йر�
	 * close.c
	 * _close: �رղ��ͷű�׼C���ļ����
	*/
...
int acl_vstream_close(ACL_VSTREAM *fp)
{
	...
	/* ���ڵ��ø����رջص�����֮ǰ�����ӹرգ������Ӱ�� iocp ���¼�����
	 * ������������ʹ�� iocp �¼�����ʱ�������ر�ʱ����� events_iocp.c ��
	 * �� stream_on_close���ú������ͷŵ� fdp->event_read/fdp->event_write
	 * �������󣬵����׽ӿ�δ�ر�ʱ�������������п��ܻᱻ iocp ʹ�ã�����
	 * �ӿڹر�ʱ��iocp �Ų���ʹ�������������е� IOCP_EVENT->overlapped ��
	 * ��Ա. ---2011.5.18, zsx
	 */
	/*
	 * 2011.5.18 �ĸĶ��������¼�����Ϊ iocp �����⣬��ͬʱ����� win32
	 * ������Ϣ��������⣬��Ȼ win32 ��Ϣ����ķ�ʽ�ڹر��׽ӿ�֮ǰ��ص�
	 * stream_on_close���ûص�Ҫ���׽ӿڱ����Ǵ򿪵ģ���Ȼ���߳����˳�ͻ��
	 * �� iocp �����⻹���� iocp ���汾��ȥ����ɣ����� iocp �����
	 * stream_on_close �У����ͷ� fdp->event_read/fdp->event_write ǰ�ر�
	 * �׽ӿڼ��ɣ��� acl_vstream_close �����Ҫ�ر��׽ӿ�ʱֻҪ���ݾ��
	 * �Ƿ���Ч���ж��Ƿ���ùرչ���. ---2011.5.19, zsx
	 */
	/*
	if (fp->read_buf != NULL)
		acl_myfree(fp->read_buf);
