...
/* ========================================================================= */
unsigned acl_hash_crc32(const void *buffer, size_t len)
{
...
acl_uint64 acl_hash_crc64(const void *buffer, size_t len)
{
	const unsigned char* buf = (const unsigned char *) buffer;
	acl_uint64 crc = INITIALCRC;

	while (len-- > 0) {
		crc = crc64_table[(int)((crc ^ *buf++) & 0xff)] ^ (crc >> 8);
	}

	return (crc);
}

unsigned acl_hash_bin(const void *key, size_t len)
{
	unsigned long h = 0;
	unsigned long g;
...
unsigned acl_hash_func2(const void *key, size_t len)
{
...
unsigned acl_hash_func3(const void *key, size_t len)
{
...
unsigned acl_hash_func4(const void *key, size_t len)
{
	const unsigned char *k = (const unsigned char *) key;
	size_t loop;
	unsigned int h;
...
unsigned acl_hash_func5(const void *key, size_t len)
{
...
/* come from squid */
unsigned acl_hash_func6(const void *key, size_t len)
{
